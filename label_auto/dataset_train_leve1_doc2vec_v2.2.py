...
def get_sigle_term_category(list):
    term_classification_list_all = []
    for i ,line in enumerate(list):
        term_most_similar_classification_amount_list = []
        numbers = i
        # print(line)
        for term in line:
            term_vector = model.infer_vector(term, alpha=0.25, min_alpha=0.1, steps=10)
            term_classification = model.docvecs.most_similar([term_vector],topn=1)
            term_most_similar_classification_amount_list.append(term_classification[0][0])
        term_classification_list_deduplication = set(term_most_similar_classification_amount_list)

        term_classification_times_dict = {}
        for item in term_classification_list_deduplication:
            term_classification_times_dict.update({item: term_most_similar_classification_amount_list.count(item)})
        term_classification_list_all.append(term_classification_times_dict)
        # print(term_classification_times_dict)
    return term_classification_list_all


def  infer_vector_for_sigle_term(list):
    term_top_vector_list_all = []
    for i ,line in enumerate(list):
        term_highest_vector_top_list = []
        numbers = i
        for term in line:
            term_vector = model.infer_vector(term, alpha=0.25, min_alpha=0.1, steps=10)
            term_classification = model.docvecs.most_similar([term_vector],topn=1)
            term_highest_vector_top_list.append(term_classification[0])
            # print(term_highest_vector_top_list)

        term_top_vector_list_all.append(term_highest_vector_top_list)
    return term_top_vector_list_all
...
if __name__=='__main__':
    ...
    # x = get_sigle_term_category(modified_test_lines)
    y = infer_vector_for_sigle_term(modified_test_lines)
    print(y[0:9])
