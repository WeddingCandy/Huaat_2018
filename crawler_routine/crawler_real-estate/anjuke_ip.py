# -*- coding:utf-8 -*-
import os, re, time, random, urllib.request,datetime,socket,math
import requests
####################
def openurl(url, head):
    ...
    if match:
        ...
        url_tmp = "http://" + url
    req = requests.get(url_tmp, headers=head)
...
def sleepRandom(b = 5, e = 10, p = "TRUE"):
    ...
    if p:
        time_log("sleep " + str(t) + " sec.")
    time.sleep(t)

####################
#print(os.getcwd())
os.chdir(r"e:\crawler")
#print(os.getcwd())
####################
data = readDictfile("anjuke_city.txt")

#des_path = "D:/python_test/URL"
des_path = r"e:\crawler\output_1"
        ...
        while next_url != "NO":
            ...
            try:
                ...
                if len(matchs_no)<1:
                    ...
                    print("time_diff : " + str(time_diff))
                    print("time_diff : " + str(time_diff))
                    print(time_diff-100<0)
                    if time_diff-100<0:
                        ttt = abs(time_diff-100) +6
                        print(ttt)
                        sleepRandom(ttt, ttt)
                    else:
                        lag_time = ( math.ceil(time_diff/100) - (time_diff/100) ) * 100 + 6
                        print(lag_time)
                        sleepRandom(lag_time, lag_time)
                        print("lag_time : " + str(lag_time))
                    # lag_time = 300+6 - time_diff
                    # print("lag_time : " + str(lag_time))
                    start_time = datetime.datetime.now()
                    print(start_time)
