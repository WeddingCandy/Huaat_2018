...
class BiRNN(object):
    ...
    def __init__(self, embedding_size, rnn_size, layer_size,
        ...
        with tf.device('/cpu:0'):
            embedding = tf.Variable(tf.truncated_normal([vocab_size, embedding_size], stddev=0.1), name='embedding',dtype=tf.float32)
            print(embedding)
        ...
        self.accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(self.targets, axis=1), tf.argmax(self.prob, axis=1)),
                                               tf.float32))
