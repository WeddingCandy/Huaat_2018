...
excel_sheet = pd.read_excel('C:/Users/thinkpad/Desktop/crawlers/labels/labels.xlsx',sheet_name = 'url',header= 0)
...
for id in range(0,length):
    try:
        req = requests.get(url=labels[id][0], headers=headers ,timeout=20)
        if req == None:
            continue
        else:
    ...
    excel_sheet_extract.ix[id:id,'label_contents'] = str(keep).replace('[','').replace("'",'').replace(']','')


path = 'C:\\Users\\thinkpad\\Desktop\\result_test2.csv'
