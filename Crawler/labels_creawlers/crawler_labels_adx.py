...
excel_sheet = pd.read_excel('C:/Users/thinkpad/Desktop/adx.xlsx',sheet_name = 'sheet1',header= 0)
...
labels_ori = "http://" + excel_sheet.iloc[:,0:1]
...
for id in range(0,length):
    try:
        req = requests.get(url=labels[id][0], headers=headers ,timeout=10)
        if req == None:
            continue
        else:
            html = req.content#.encode(req.encoding).decode(req.encoding)
    ...
    #     meta_tostring)
    pattern = "<title>([^&<]+)</title>?"
    x = str(re.findall(pattern,meta_keyword))
    print('第%s个label已完成，成功次数%d' %(id+1,count_success))

    excel_sheet.ix[id:id,'label_contents'] = x.replace('[','').replace(']','').replace("'",'')


path = 'C:\\Users\\thinkpad\\Desktop\\result_adx2.xls'
excel_sheet.to_excel(path,encoding='utf-8' )
print('done')
